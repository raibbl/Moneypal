...
// import { AppLoading, Asset, Font, Icon } from 'expo'; to fix font problem later
export default class AddScreen extends React.Component {
  ...
  SavetoGlobalTransaction = async () => {
    ...
    let { year, month, day, name, amount, budget, longitude, latitude, count } =
      this.state;
    ...
    return (
      <PaperProvider>
        <React.Fragment>
          <Appbar>
            <Appbar.BackAction
              onPress={() => this.props.navigation.navigate("Home", { x })}
            />
          ...
          >
            ...
            />
            <Button onPress={this.SavetoGlobalTransaction}>Save expense</Button>
            <TextInput
              style={{ flex: 1 }}
            ...
            <Button onPress={this.saveDate}>Date of transaction</Button>
            ...
            >
              <Button disabled={false}>Category</Button>
              <Picker
                style={{ height: 50, width: 100 }}
                onValueChange={
                  (itemValue, itemIndex) => this.setState({ budget: itemValue })
